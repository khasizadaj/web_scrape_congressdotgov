...
@logger.catch()
def main(congresses: List[str], pages_to_scrape_dict: Dict[str, int]):
    params = {"q": "", "pageSize": 100, "page": 1}

    for congress in congresses:
        print("Current congress: " + congress)

        params["q"] = '{"source":"legislation","congress":' + congress + "}"

        # initializing empty file with headers
        save(congress, init=True)

        pages_to_scrape = pages_to_scrape_dict[congress]
        for page_num in range(pages_to_scrape[0], pages_to_scrape[1] + 1):
            ...
            congress_url = get_congress_url(params)

            # getting the html text and reading it with beautiful soup
            congress_response: Response = requests.get(congress_url, params=params)
            time.sleep(1)

            soup = BeautifulSoup(congress_response.text, "html.parser")
            ...
            for index_bill_list_item, bill_list_item in enumerate(bill_list_items):
                ...
                (
                    curr_number_of_cosponsors,
                    curr_average_year_of_services,
            ...
            # Step Eight: using zip, create a list in which each element is a "row" of data,
            # and then with csv.writer create the csv file storing the scraped dataset
...
def save(congress: str, dataset=None, init=False) -> None:
    """
    Function creates file for each congress to save.

    Args:
        congress: string that represents congress
    ...
    path_to_save = f"{config.OUTPUT_DIR}/congress_{congress}.{config.OUTPUT_FORMAT}"
...
def process_cosponsor_information(bill_soup: BeautifulSoup) -> Tuple[float, int]:
    ...
    cosponsors_soup = BeautifulSoup(cosponsor_response.text, "html.parser")
...
def get_characteristics_of_cosponsor(cosponsor_link):
    ...
    cosponsor_soup = BeautifulSoup(cosponsor_response.text, "html.parser")
...
if __name__ == "__main__":
    ...
    congresses = ["115"]
    main(congresses, pages_to_scrape)
